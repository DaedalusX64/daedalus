...
,	mCurrentDisplayOption( mCurrentOption )
{
	...
	mOptionComponents[ MO_ROMS ]			= CRomSelectorComponent::Create( mpContext, [this](const char * romName) { OnRomSelected(romName); } );
	mOptionComponents[ MO_SELECTED_ROM ]	= mSelectedRomComponent;
	mOptionComponents[ MO_SAVESTATES ]		= CSavestateSelectorComponent::Create( mpContext, CSavestateSelectorComponent::AT_LOADING, [this](const char * stateName) {OnSavestateSelected(stateName); }, 0 );
