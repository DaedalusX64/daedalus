...
#endif
  u32 addr(command.Abi1LoadBuffer
...
void LOADBUFF2(AudioHLECommand command) {
  ...
  u32 src(command.Abi2LoadBuffer
              .SrcAddr); // + gAudioHLEState.Segments[(command.cmd1>>24)&0xf];
...
#endif
  u32 addr(command.Abi1SaveBuffer
               .Address); // + gAudioHLEState.Segments[(command.cmd1>>24)&0xf];
...
#endif
  // Needs accuracy verification...
  u32 dst(command.Abi2SaveBuffer
              .DstAddr); // + gAudioHLEState.Segments[(command.cmd1>>24)&0xf];
...
void SETLOOP(AudioHLECommand command) {
  ...
  u32 loopval(
      command.Abi1SetLoop
          .LoopVal); // + gAudioHLEState.Segments[(command.cmd1>>24)&0xf];
