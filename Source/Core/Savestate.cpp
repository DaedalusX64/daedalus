...
public:
	...
	{
		write(g_pMemoryBuffers[buffernum], size ? pspFpuMin(MemoryRegionSizes[buffernum], size) : MemoryRegionSizes[buffernum]);
	...
	{
		write((u8*)g_pMemoryBuffers[buffernum] + offset, size ? pspFpuMin(MemoryRegionSizes[buffernum], size) : MemoryRegionSizes[buffernum]);
		if(size > MemoryRegionSizes[buffernum])
			skip(size - MemoryRegionSizes[buffernum]);
	}
};
...
public:
	...
	virtual void skip( size_t size )
	{
		...
		if( mStream.IsOpen() )
		{
			...
			while( remaining > 0 )
			{
				u32		max_to_write( pspFpuMin( remaining, MAX_ZEROES ) );
...
public:
	...
	{
		read(g_pMemoryBuffers[buffernum], size ? pspFpuMin(MemoryRegionSizes[buffernum], size) : MemoryRegionSizes[buffernum]);
		if(size > MemoryRegionSizes[buffernum])
			skip(size - MemoryRegionSizes[buffernum]);
	}
	...
	{
		read((u8*)g_pMemoryBuffers[buffernum] + offset, size ? pspFpuMin(MemoryRegionSizes[buffernum], size) : MemoryRegionSizes[buffernum]);
		if(size > MemoryRegionSizes[buffernum])
			skip(size - MemoryRegionSizes[buffernum]);
	}
...
public:
	...
	virtual void skip( size_t size )
	{
		if( mStream.IsOpen() )
		{
			...
			while( remaining > 0 )
			{
				u32		max_to_read( pspFpuMin( remaining, BUFFER_SIZE ) );
